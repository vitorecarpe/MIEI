...
/** \brief <b>Função combosValoresPoker</b>\n
...
int *combosValoresPoker(MAO cartas) {
...
independentemente do naipe - ). Esta função recorre à ordenação da função combosValoresPoker da seleção e identifica quando 5 cartas
...
int jogadaPENTAstraight(MAO selecao){
    int n,i;
    int *comboVP;
    comboVP=combosValoresPoker(selecao);
    for(n=0,i=0;i<14;i++)
        if(n<5) {    
            if(comboVP[i]>0) n++;
            else n=0;
...
/** \brief <b>Função jogadaPENTAstraightFlush</b>\n
Função que reconhece um Straight Flush como uma jogada válida. 
@param selecao Recebe uma mão da seleção (long long integer).
@return Função que através das cartas selecionadas diz se estamos perante um Straight Flush (Ex: 5♠ 6♠ 7♠ 8♠ 9♠ - uma conjugação de um 
Straight - 5 cartas com valores seguidos - e de um Flush - 5 cartas do mesmo naipe -). Esta função recorre à ordenação da função 
combosNaipes da seleção e verifica se as cartas selecionadas são do mesmo naipe unicamente. Posteriormente, pega nessa seleção e 
verifica se os valores são sequenciais, através da função combosValoresPoker. Caso estas condições se verifiquem, devolve 1 (return 1)
ou seja, a jogada é válida, caso contrário, devolve 0.\n
<b>Exemplo:</b>\n 9♥ 10♥ J♥ Q♥ K♥ na seleção devolve 1.\n 9♥ 10♥ J♥ Q♥ A♥ na seleção devolve 0.\n 9♥ 10♥ J♥ Q♥ K♠ na seleção
devolve 0.
\note Esta função pode também ser usada para testar se existe um Straight Flush na mão de um bot.*/
int jogadaPENTAstraightFlush(MAO selecao){
    ...
    for(x=0,n=0;n<4;n++){
        if(comboN[n]>=5){
            ...
            comboVP=combosValoresPoker(tempHand);
            for(vp=0;vp<14;vp++)
                if(x<5) {    
                    if(comboVP[vp]>=1) x++;
                    else x=0;
...
/** \brief <b>Função selecionaPENTAstraight</b>\n
Função atualiza o estado para jogar um Straight efetivamente. Esta função só é chamada quando é verificado na jogadaPENTAstraight que o jogador
"p" tem, de facto, um Straight na sua mão.
@param ESTADO Recebe o estado atual do jogo.
@param p O número identificativo do jogador.
@return Devolve a mão que é pretendida ser jogada.*/
MAO selecionaPENTAstraight(ESTADO e,int p) {
    int n=0,v=0,vp=0;
    int i=0,x=0;
    MAO selecao=0;
    int *comboVP;
    comboVP=combosValoresPoker(e.mao[p]);
...
/** \brief <b>Função selecionaPENTAflush</b>\n
Função atualiza o estado para jogar um Flush efetivamente. Esta função só é chamada quando é verificado na jogadaPENTAflush que o jogador
"p" tem, de facto, um Flush na sua mão.
@param ESTADO Recebe o estado atual do jogo.
@param p O número identificativo do jogador.
@return Devolve a mão que é pretendida ser jogada.*/
...
/** \brief <b>Função selecionaPENTAfullhouse</b>\n
Função atualiza o estado para jogar um Full House efetivamente. Esta função só é chamada quando é verificado na jogadaPENTAfullhouse que o jogador
"p" tem, de facto, um Full House na sua mão.
@param ESTADO Recebe o estado atual do jogo.
@param p O número identificativo do jogador.
@return Devolve a mão que é pretendida ser jogada.*/
...
/** \brief <b>Função selecionaPENTA4ofAkind</b>\n
Função atualiza o estado para jogar um Four Of A Kind efetivamente. Esta função só é chamada quando é verificado na jogadaPENTA4ofAkind que o jogador
"p" tem, de facto, um Four Of A Kind na sua mão.
@param ESTADO Recebe o estado atual do jogo.
@param p O número identificativo do jogador.
@return Devolve a mão que é pretendida ser jogada.*/
...
/** \brief <b>Função selecionaPENTAstraightFlush</b>\n
Função atualiza o estado para jogar um Straight Flush efetivamente. Esta função só é chamada quando é verificado na jogadaPENTAstraightFlush que o jogador
"p" tem, de facto, um Straight Flush na sua mão.
@param ESTADO Recebe o estado atual do jogo.
@param p O número identificativo do jogador.
@return Devolve a mão que é pretendida ser jogada.*/
MAO selecionaPENTAstraightFlush(ESTADO e,int p) {
    ...
    comboVPtemp=combosValoresPoker(tempHand);
    for(vp=0;vp<14;vp++)
        if(i<5){
            if(comboVPtemp[vp]>0){
                switch(vp){
...
/** \brief <b>Função jogadaValida</b>\n
Função que verifica se as cartas selecionadas são uma jogada válida.\n
@param selecao A seleção
@param ncartas Número de cartas
@return Retorna 1 ou 0, de acordo com a validade da jogada.*/
...
/** \brief <b>Função jogadaValida</b>\n
Função que verifica se as cartas selecionadas são uma jogada válida superior à última jogada.\n
@param selecao A seleção.
@param ultimajogada A última jogada do jogador inteligente.
@param ncartasselecao Número de cartas da seleção.
@param ncartascombo Número de cartas da combinação.
@param ncartasPENTAtype Número da valoração da jogada de 5 cartas (51, 52, 53, 54 ou 55).
@return Retorna 1 se a jogada é válida e superior à última ou retorna 0 em caso contrário.*/
...
/** \brief <b>Função jogadaSTARTINGstraight</b>\n
Função executada no ínicio da partida que procura um Straight que contenha um 3♦ numa mão de qualquer jogador.
@param selecao Recebe uma mão da seleção (long long integer).
@return Esta função retorna 1 caso encontre um Straight com 3♦. Caso contrário, devolve 0.\n
<b>Exemplo:</b>\n 3♦ 4♥ 5♥ 6♠ 7♦ devolve 1.\n 3♣ 4♥ 5♥ 6♠ 7♦ devolve 0.\n 3♦ 4♥ 5♥ 6♠ 8♦ devolve 0.*/
...
/** \brief <b>Função jogadaSTARTINGstraight</b>\n
Função executada no ínicio da partida que procura um Flush que contenha um 3♦ numa mão de qualquer jogador.
@param selecao Recebe uma mão da seleção (long long integer).
@return Esta função retorna 1 caso encontre um Flush com 3♦. Caso contrário, devolve 0.\n
<b>Exemplo:</b>\n 3♦ 9♦ 10♦ J♦ A♦ devolve 1.\n 3♣ 8♣ 10♣ K♣ A♣ devolve 0.\n 3♦ J♦ Q♦ A♦ K♥ devolve 0.*/
...
/** \brief <b>Função jogadaSTARTINGfullhouse</b>\n
Função executada no ínicio da partida que procura um Full House que contenha um 3♦ numa mão de qualquer jogador.
@param selecao Recebe uma mão da seleção (long long integer).
@return Esta função retorna 1 caso encontre um Full House com 3♦. Caso contrário, devolve 0.\n
<b>Exemplo:</b>\n 3♦ 3♠ K♣ K♦ K♥ devolve 1.\n 3♣ 3♠ K♣ K♦ K♥ devolve 0.\n 3♣ K♦ K♣ K♥ K♠ devolve 0.*/
...
/** \brief <b>Função jogadaSTARTING4ofAkind</b>\n
Função executada no ínicio da partida que procura um Four Of A Kind que contenha um 3♦ numa mão de qualquer jogador.
@param selecao Recebe uma mão da seleção (long long integer).
@return Esta função retorna 1 caso encontre um Four Of A Kind com 3♦. Caso contrário, devolve 0.\n
<b>Exemplo:</b>\n 3♦ K♦ K♣ K♥ K♠ devolve 1.\n 3♣ K♦ K♣ K♥ K♠ devolve 0.\n 3♦ 3♠ K♣ K♦ K♥ devolve 0.*/
...
/** \brief <b>Função jogadaSTARTINGstraightFlush</b>\n
Função executada no ínicio da partida que procura um Straight Flush que contenha um 3♦ numa mão de qualquer jogador.
@param selecao Recebe uma mão da seleção (long long integer).
@return Esta função retorna 1 caso encontre um Straight Flush com 3♦. Caso contrário, devolve 0.\n
<b>Exemplo:</b>\n 3♦ 4♦ 5♦ 6♦ 7♦ devolve 1.\n 3♦ 4♦ 5♥ 6♦ 7♦ devolve 0.\n 3♥ 4♥ 5♥ 6♥ 7♥ devolve 0.*/
...
/** \brief <b>Função selecionaSTARTINGstraight</b>\n
Função executada no ínicio da partida que joga um Straight que contenha um 3♦ numa mão de qualquer jogador. Esta função só é chamada quando existe realmente
um Straight com 3♦ no jogador "p".
@param ESTADO Recebe o estado do jogo.
@param p O número identificativo do jogador inteligente (incluíndo botão de ajuda).
@return Devolve a mão selecionada que contém um Straight com 3♦.*/
...
/** \brief <b>Função selecionaSTARTINGflush</b>\n
Função executada no ínicio da partida que joga um Flush que contenha um 3♦ numa mão de qualquer jogador. Esta função só é chamada quando existe realmente
um Flush com 3♦ no jogador "p".
@param ESTADO Recebe o estado do jogo.
@param p O número identificativo do jogador inteligente (incluíndo botão de ajuda).
@return Devolve a mão selecionada que contém um Flush com 3♦.*/
...
/** \brief <b>Função selecionaSTARTINGfullhouse</b>\n
Função executada no ínicio da partida que joga um Full House que contenha um 3♦ numa mão de qualquer jogador. Esta função só é chamada quando existe realmente
um Full House com 3♦ no jogador "p".
@param ESTADO Recebe o estado do jogo.
@param p O número identificativo do jogador inteligente (incluíndo botão de ajuda).
@return Devolve a mão selecionada que contém um Full House com 3♦.*/
...
/** \brief <b>Função selecionaSTARTING4ofAkind</b>\n
Função executada no ínicio da partida que joga um Four Of A Kind que contenha um 3♦ numa mão de qualquer jogador. Esta função só é chamada quando existe realmente
um Four Of A Kind com 3♦ no jogador "p".
@param ESTADO Recebe o estado do jogo.
@param p O número identificativo do jogador inteligente (incluíndo botão de ajuda).
@return Devolve a mão selecionada que contém um Four Of A Kind com 3♦.*/
...
/** \brief <b>Função selecionaSTARTINGstraightflush</b>\n
Função executada no ínicio da partida que joga um Straight Flush que contenha um 3♦ numa mão de qualquer jogador. Esta função só é chamada quando existe realmente
um Straight Flush com 3♦ no jogador "p".
@param ESTADO Recebe o estado do jogo.
@param p O número identificativo do jogador inteligente (incluíndo botão de ajuda).
@return Devolve a mão selecionada que contém um Straight Flush com 3♦.*/
...
/** \brief <b>Função playWith3</b>\n
Função que é chamada nas funções botPlayx quando este é o primeiro a jogar (tem de jogar o 3♦) Esta função executa a melhor jogada possível com o 3♦.\n
@param ESTADO O estado atual
@param p Número identificativo do jogador.
@return Retorna o novo estado da partida.*/
...
/** \brief <b>Função botPlay1</b>\n
Função responsável por chamar todas as outras funções auxiliares que fornecem a inteligência artificial ao jogador 1 e devolver o novo estado da
partida.\n
@param ESTADO O estado atual do jogo.
@param pickFirst Inteiro que é o responsável por definir se este jogador é o que tem o 3♦ no seu baralho (pickFirst=1 então 3♦ está no seu baralho,
caso contrário, pickFirst=0).
@return Retorna o novo estado do jogo.*/
...
/** \brief <b>Função botPlay2</b>\n
Função responsável por chamar todas as outras funções auxiliares que fornecem a inteligência artificial ao jogador 2 e devolver o novo estado da
partida.\n
@param ESTADO O estado atual do jogo.
@param pickFirst Inteiro que é o responsável por definir se este jogador é o que tem o 3♦ no seu baralho (pickFirst=1 então 3♦ está no seu baralho,
caso contrário, pickFirst=0).
...
/** \brief <b>Função botPlay3</b>\n
Função responsável por chamar todas as outras funções auxiliares que fornecem a inteligência artificial ao jogador 3 e devolver o novo estado da
partida.\n
@param ESTADO O estado atual do jogo.
@param pickFirst Inteiro que é o responsável por definir se este jogador é o que tem o 3♦ no seu baralho (pickFirst=1 então 3♦ está no seu baralho,
caso contrário, pickFirst=0).
...
/** \brief <b>Função passar</b>\n
Função que atualiza o estado da partida para passar a jogada para o próximo jogador.\n
@param ESTADO O estado atual
@return Retorna o novo estado da partida.*/
...
/** \brief <b>Função jogar</b>\n
Função que atualiza o estado da partida para jogar as cartas da seleção, quando é clicado o botão "Jogar".\n
@param ESTADO O estado atual
@return Retorna o novo estado da partida.*/
    ...
    for(v = 0; v < 13; v++) {
        for(n = 0; n < 4; n++){
            ...
            if(carta_existe(e.ultimajogada[0],n,v)){
                imprime_carta_naoclicavel(X,355,n,v); X+=25;
            }
        }
    }
...
/** \brief <b>Função imprime_jogar</b>\n
Função responsável por imprimir os botões de Jogar. Esta função pode imprimir dois botões. Um botão ativo que é impresso no ecrã quando a jogada selecionada
é válida. Ou pode imprimir um botão inativo quando não é selecionada uma jogada válida.\n
@param ESTADO O estado atual*/
...
/** \brief <b>Função imprime_passar</b>\n
Função responsável por imprimir o botão de Passar.\n
@param ESTADO O estado atual*/
...
/** \brief <b>Função imprime_BotaoAjuda</b>\n
Função responsável por imprimir o botão de Ajuda, botão esse que dá uma dica de uma jogada possível ao utilizador.\n
@param ESTADO O estado atual*/
...
/** \brief <b>Função imprime_NaipeValor</b>\n
Função responsável por imprimir os botões que alteram a ordenação das cartas no baralho do utilizador, ora pelo seu valor, ou pelo naipe.\n
@param ESTADO O estado atual*/
...
/** \brief <b>Função imprime_caixas_pontuacao</b>\n
Função responsável por imprimir as caixas onde se encontra a pontuação de cada jogador e o número de cartas que tem no seu baralho.\n
@param ESTADO O estado atual*/
...
/** \brief <b>Função imprimeBotoes</b>\n
Função responsável por imprimir todos os botões do jogo, chamando todas as funções responsáveis por imprimir cada botão ou conjunto de botões.\n
@param ESTADO O estado atual*/
...
/** \brief <b>Função imprime</b>\n
Função responsável por imprimir diversas imagens no ecrã:\n
• Imprime o baralho do utilizador e dos jogadores inteligentes (bots);\n
• Imprime as cartas jogadas dos bots;\n
• Imprime o ecrã de vitória quando o utilizador venceu o jogo e o ecrã de derrota quando um bot qualquer vence a partida;\n
• Imprime os gráficos decorativos dos bots;\n
• Chama a função imprimeBotoes.\n
@param ESTADO O estado atual*/
...
/** \brief <b>Função find3D</b>\n
Função responsável por detetar o jogador que tem o 3♦ na sua mão.
@param ESTADO O estado atual.
@return Retorna o novo estado da partida.
*/ 
ESTADO find3D(ESTADO e){
    int p=0,x=0;
    for(p=0;p<4;p++)
        if(carta_existe(e.mao[p],0,0)) x=p; /* x guarda o numero do jogador que tem o 3♦ */
    switch (x){
        case 0:
        break;
        case 1: 
...
/** \brief <b>Função distribuir</b>\n
Função responsável por distribuir as cartas aleatoriamente (baralhar) pelos 4 jogadores. 13 a cada um respetivamente.
@param ESTADO O estado atual
...
/** \brief <b>Função parse</b>\n
Trata os argumentos da CGI.
@param query A query que é passada à cgi-bin
@return Esta função recebe a query que é passada à cgi-bin e trata-a.
