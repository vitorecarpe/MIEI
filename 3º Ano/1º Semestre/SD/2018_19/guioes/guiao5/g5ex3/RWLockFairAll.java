...
public class RWLockFairAll implements RWLockInterface {
	...
	int writersWaiting; 	//número de escritores que pretendem adquirir o lock
	int readersWaiting; 	//número de leitores que pretendem adquirir o lock	
	...
	public RWLockFairAll(){
		...
		this.readersCond = lock.newCondition();
		this.writersCond = lock.newCondition();
		this.readers = this.writer = 0;
		this.writersPriority = this.readersPriority = 0;
		this.readersWaiting = this.writersWaiting = 0;
	}
	...
	public void readLock(){
		...
		try{
			...
			readersWaiting++;
			while( this.writer==1 || this.writersWaiting>0 && this.readersPriority>=this.MAXPRIORITY ){ 
				readersCond.await();
			}
	...
	public void writeLock(){
		...
		try{
			...
			while( this.readers != 0 || this.writer != 0 || 
					(this.readersWaiting>0 && this.writersPriority>=this.MAXPRIORITY) ){	
				writersCond.await();
			}
